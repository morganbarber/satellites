...
class TestStatefulSession(unittest.TestCase):
    """Integration test suite for StatefulSession engine."""

    def test_stateful_session_sequence_udp(self):
        """Tests 2-step sequence execution against local UDP mock server."""
        ...
        with StatefulSession(
            "127.0.0.1", port, protocol="udp", scid=12, vcid=4, apid=83, bypass=0
        ) as session:
            result = session.run_sequence(
                start_counter=0, sync_payload=b"BEGIN", next_payload=b"GETFLAG", fmt_style="auto"
            )
